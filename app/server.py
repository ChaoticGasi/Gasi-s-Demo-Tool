...
PROJECT_ROOT = Path(sys.executable).resolve().parent if FROZEN else ROOT.parent
STATIC = ROOT / "static"
RECORDINGS = PROJECT_ROOT / "recordings"
RUNTIME_ROOT = Path(os.environ.get("LOCALAPPDATA", str(ROOT / "runtime"))) / "CS2DemoRecorder"
GENERATED = (RUNTIME_ROOT / "generated") if FROZEN else (ROOT / "generated")
...
DEMO_READY_MARKER_NAME = "cs2_demo_loaded_marker.cfg"
DEMO_READY_LOG_NAME = "cs2_demo_loaded_marker.log"
DEMO_READY_DUMP_NAME = "cs2_demo_loaded_marker"
DEMO_READY_TEXT = "CS2_DEMO_TOOL_DEMO_LOADED"
...
def build_commands(payload):
    ...
    demo_ready_log = RUNTIME_ROOT / DEMO_READY_LOG_NAME
    demo_ready_log_cfg = str(demo_ready_log).replace("\\", "/")
    ...
    commands.extend([
        f'con_logfile "{demo_ready_log_cfg}"',
    ...
    for ready_tick in (64, 128, 256, 512):
        commands.append(f'mirv_cmd addAtTick {ready_tick} "echo {DEMO_READY_TEXT}; condump {DEMO_READY_DUMP_NAME}"')
    ...
    if pov_commands:
        ...
        commands.append('alias cs2dt_pov ""')
        for tick in (32, 64, 128, 256, 512):
    ...
    commands.extend([
        ...
        f'alias cs2dt_demo_back "demo_goto -{demo_jump_ticks} relative"',
        'alias cs2dt_demo_pause "demo_togglepause"',
        f'alias cs2dt_demo_forward "demo_goto {demo_jump_ticks} relative"',
        "alias rec cs2dt_rec_start",
        "con_enable 1",
        'bind "^" "toggleconsole"',
        'bind "`" "toggleconsole"',
        'bind scancode53 "toggleconsole"',
        'bind F1 "cs2dt_demo_back"',
        'bind F2 "cs2dt_demo_pause"',
        'bind F3 "cs2dt_demo_forward"',
        'bind F7 "cs2dt_hud; cs2dt_pov"',
        "bind F8 \"demoui; gameui_hide\"",
        'bind F9 "cs2dt_hud"',
        'bind F10 "rec"',
        'bind F11 "exec cs2_demo_refresh"',
        'bind F4 "exec cs2_demo_overlay"',
    ])
...
def write_auto_cfg(payload):
    ...
    demo_ready_marker = cfg_dir / DEMO_READY_MARKER_NAME
    demo_ready_log = RUNTIME_ROOT / DEMO_READY_LOG_NAME
    demo_ready_logs = [demo_ready_log]
    demo_ready_dump_dirs = [RUNTIME_ROOT]
    cs2_candidate = payload.get("cs2Path") or find_cs2_path()
    if cs2_candidate:
        cs2_path = Path(cs2_candidate).expanduser()
        game_dir = next((parent for parent in cs2_path.parents if parent.name.lower() == "game"), None)
        if game_dir:
            csgo_dir = game_dir / "csgo"
            demo_ready_logs.append(csgo_dir / "console.log")
            demo_ready_dump_dirs.append(csgo_dir)
    demo_ready_marker.unlink(missing_ok=True)
    for log_path in demo_ready_logs:
        try:
    ...
    data["demoReadyMarker"] = str(demo_ready_marker)
    data["demoReadyLog"] = str(demo_ready_log)
    data["demoReadyLogs"] = [str(path) for path in demo_ready_logs]
    data["demoReadyDumpDirs"] = [str(path) for path in demo_ready_dump_dirs]
    data["demoReadyText"] = DEMO_READY_TEXT
...
def write_overlay_cfg(commands, cfg_dir=None):
    cfg_dir = cfg_dir or (RUNTIME_ROOT / "mmcfg" / "cfg")
    cfg_dir.mkdir(parents=True, exist_ok=True)
    clean_commands = [str(command).strip() for command in commands if str(command).strip()]
    clean_commands.append("echo CS2 Demo Tool overlay command applied")
    return write_chained_cfgs(cfg_dir, "cs2_demo_overlay", clean_commands)
